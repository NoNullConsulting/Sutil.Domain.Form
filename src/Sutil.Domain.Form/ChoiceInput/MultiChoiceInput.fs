...
module ViewModel = 
    ...
    let update msg (model : ViewModel<'T> list) = 
        match msg with
        | Toggle  index -> 
            model.setI(index, fun v -> { v with Selected = not v.Selected})

        | SetValue (i, value) -> 
            model.setI(i, fun v -> {v with Value = value})
...
module MultiChoiceInput = 
    ...
    let inline render controls =
        let viewModel, dispatch = Store.makeElmishSimple (ViewModel.init) ViewModel.update ignore controls.Values

        let getIsSelected index = 
            viewModel.map( List.item index >> fun ({Selected = s}) -> s)
            |> Store.distinct
