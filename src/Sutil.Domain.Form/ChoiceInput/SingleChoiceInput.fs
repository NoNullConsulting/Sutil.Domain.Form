...
type SingleChoiceInput<'TValue, 'TResult when 'TValue: equality> =
    ...
    interface IFormElement<SingleChoiceInput<'TValue, 'TResult>> with
        member controls.UpdateFormContainer(mapper) =
            { controls with
                ContainerProps = mapper controls.ContainerProps }
...
type SingleChoiceInputViewModel<'T, 'R> =
    ...
    member this.IsChecked index =
        this.Choices |> List.item index |> (fun c -> c.Selected)
    ...
    let update msg model =
        ...
        | Toggle index ->
            let nextChoices =
                List.mapi
                    (fun i v ->
                        { Selected = index = i && not v.Selected
                          Value = v.Value })
                    model.Choices
...
module SingleChoiceInput =
    ...
    let inline isRequired (controls: SingleChoiceInput<_, _>) = { controls with IsRequired = true }
    ...
    type SingleChoiceInput<'TValue, 'TResult when 'TValue: equality> with
        ...
        member inline controls.withParser(parser) = withValueParser parser controls

        member inline controls.withParser(parser) = withOptionParser parser controls

        member inline controls.isRequired() = isRequired controls

        member inline controls.render() = render controls
