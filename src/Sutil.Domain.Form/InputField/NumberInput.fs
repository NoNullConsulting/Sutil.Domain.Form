...
module NumberInputHelper = 
    ...
    let isValidNumberOrSkip parser (store : Store<string>) str =
        ...
        |> fun isValid -> 
            if isValid then Store.set store str else Store.set store (Store.current store) 
...
type NumberInput<'TResult when 'TResult : equality> = 
    ...
    with 
        member inline model.setParser(parser) = 
        ...
        member inline model.setAutoCompleateName( str ) = 
            { model with AutoCompleate = Some str }
        
        member inline model.placeholder( str ) = 
            { model with Placeholder = Some str }
        ...
        member inline model.render () : FormElement<_> =
            ...
            let view = model.ContainerProps.render [
                ...
                Html.input [
                    Attr.typeNumber
                    Bind.attr( "value", parseFromString >> Store.set result)
                ]
                Html.div [
